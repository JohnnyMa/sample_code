...
function setColorScheme(color_scheme) {
  jQuery('#color-scheme').attr('href', 'css/color-schemes/' + color_scheme + '.css?rand=' + Math.floor(Math.random() * 999999));
  /* cache bust */
}
...
jQuery(document).ready(function($) {
  ...
  // items causing additional rows trigger pagination
  ...
  function updatePortfolio(filter_category, page, no_quicksand) {
    ...
    if (!$('#portfolio-list-temp').length) {
      ...
      // empty temp first in case already existed
    }
  
    // Copy LI's that belong to clicked category from master list to temporary list
    var filter_selector = '#portfolio-list-master li';
    // all
    ...
    portfolio_filtered_urls.length = 0;
    // empty first
    ...
    $('#portfolio-page-current').text(page);
    // page numbering
  ...
  $(".portfolio-thumbs").delegate('li', 'mouseenter', function() {
    ...
    if (loading_effects_done) {// don't let hover before page load effect is done
      ...
      $('.portfolio-thumb-overlay', this).stop(true, true)// helps prevent overlays from getting stuck on super-fast mouse outs
      .fadeIn(fade_speed(300)).css('display', 'table');
      // to assist w/vertical centering
  ...
  if (!$('#portfolio-list-master').length) {
    ...
    // no category, page 1, no Quicksand effect
  ...
  /* after image/video changed */ changepicturecallback: function() {
    ...
    if (!lightbox_opened) {
      ...
      if ($('#portfolio-page-total').text() > 1) {
        ...
        if (portfolio_item_page != portfolio_current_page) {// yep, we're on a new page
          ...
          updatePortfolio(filter_category, portfolio_item_page, true);
          // no Quicksand effect, not seen
        }
  ...
  callback: function() {
    ...
    if (portfolio_section_id) {
      ...
      // first remove hash, in case it is same as current, because then position will not change
    ...
    updatePortfolio($('.portfolio-filter-selected').attr('data-category'), $('#portfolio-page-current').text(), false);
    // do use Quicksand effect, so that filter bar moves into proper place
  ...
  $(".portfolio-thumbs").delegate('a', 'click', function(event) {
    ...
    // it will be counted as opened after we go to desired image
    portfolio_item_index = (($('#portfolio-page-current').text() - 1) * max_thumbs_per_page) + $(this).parent('li').index();
    // index on current page + number of items on pages before it
  ...
  var start_section_id = location.hash && $(location.hash).length ? location.hash.replace('#', '') : $('#sections > section:first-child').attr('id');
  // if no existing section given in hash, use first section
  ...
  var loading_scroll_done = false;
  // disable menu/scroll until scroll in is done
  ...
  function loadContent(phase) {
    ...
    if (phase == 1 || !phase) {
      ...
      $('#loading-screen').hide();
      // dont fade or performance may get chunky
        ...
        if (loading_scroll_done) {// don't let menu/scroll work until top section scrolls in
          $.smoothScroll({
            ...
            beforeScroll : function() {
              this.stop();
              // let animations be interrupted with new target
            },
            afterScroll : function() {
              if (ie != 7) {// IE7 can't handle this nicely
                window.location.hash = hash;
                // now set the hash so they can copy/paste URL to section
              }
            }
          });
        }
  ...
  // Google Map (fired after portfolio loads for performance)
    function initMap() {
      ...
      if ($('#google-map').length) {
        ...
        // Custom Marker
        var image = new google.maps.MarkerImage('images/map-icon.png', new google.maps.Size(26, 26), new google.maps.Point(0, 0), new google.maps.Point(13, 26));
        var shadow = new google.maps.MarkerImage('images/map-icon-shadow.png', new google.maps.Size(40, 26), new google.maps.Point(0, 0), new google.maps.Point(13, 26));
